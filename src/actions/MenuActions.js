...
export const addItems=(item)=>async (dispatch)=>{
    ...
    try {
        const response=await axios.post('http://51.20.194.197:5002/api/items/additem',item)
...
export const deleteItem=(itemId)=>async (dispatch)=>{
    console.log({itemId})
    try {
        const response=await axios.post('http://51.20.194.197:5002/api/items/deleteitem',{itemId})
...
export const filterItem=(searchkey,category)=>async dispatch=>{
    ...
    dispatch({type:'GET_ITEMS_REQ'})
    try{
        const res=await axios.get("http://51.20.194.197:5002/api/items/getallitems")
